...
def _describe_files(version: dict[str, Any]) -> list[CivitaiFile]:
    ...
    previews = _describe_previews(version)
    # The first still image is what the site shows as its thumbnail, and what the model
    # managers expect to find as `<model>.preview.png`.
    preview = next(
        (p["url"] for p in previews if p["type"] == "image"), None
    )
...
# How many sample images are kept. Popular models publish dozens; the first handful shows
# what the thing looks like, and the rest are the same look again at the cost of a row in
# the queue database for every file of the version.
PREVIEW_LIMIT = 8
# A prompt is worth keeping whole; a prompt someone pasted a novel into is not worth
# carrying in every task row forever.
MAX_PROMPT = 2000

# Civitai's generation payload, under the names the rest of the app uses. Anything else in
# there — resource lists, extension settings, hashes of the loras involved — belongs to the
# picture rather than to the model, and is left behind.
_GENERATION = {
    "prompt": "prompt",
...
def _describe_previews(version: dict[str, Any]) -> list[dict[str, Any]]:
    """The version's sample images, with the prompt that made each one.

    The prompt is the point. A LoRA arrives with trigger words that say which tokens wake it
    up and nothing at all about what a working prompt around them looks like; the sample
    images were generated by someone who knew, and Civitai publishes their settings.
    """
    ...
    for image in version.get("images") or []:
        ...
        described.append(
            {
                "url": str(url),
                # Increasingly not an image: video previews arrive in the same array, and
                # a page that renders one in an <img> shows a broken icon.
                "type": "video" if image.get("type") == "video" else "image",
                "width": image.get("width"),
                "height": image.get("height"),
                "nsfw": _image_is_nsfw(image),
                "meta": _generation_meta(image.get("meta")),
            }
        )
        if len(described) >= PREVIEW_LIMIT:
            break
    return described


def _image_is_nsfw(image: dict[str, Any]) -> bool:
    """Whether a sample should be covered until it is asked for.

    Two fields, because the API carries both: the numeric `nsfwLevel` it uses now, and the
    older word — "None", "Soft", "Mature", "X" — that older payloads and the cached ones
    ...
    if isinstance(flag, str):
        return flag.strip().lower() not in ("", "none")
    return bool(flag)


def _generation_meta(meta: Any) -> dict[str, Any]:
    if not isinstance(meta, dict):
        return {}
    kept: dict[str, Any] = {}
    for source, name in _GENERATION.items():
        value = meta.get(source)
        if value in (None, "", []):
            continue
        kept[name] = str(value)[:MAX_PROMPT] if isinstance(value, str) else value
    return kept
